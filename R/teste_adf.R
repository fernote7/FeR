...
teste_adf = function(serie, lag_max){
    ...
    if(adf@teststat[1] < adf@cval[1,2]){
        ...
        # diferente de testar H0: a1 = g = 0 (teste F). Optamos por realizar somente o segundo teste.
        # Rejeitamos H0 se phi3 > cval (5% nesse caso) e concluímos que existe tendência determinística.
        if(adf@teststat[3] > adf@cval[3,2]){ #Não
            ...
            if(adf@teststat[1] < adf@cval[1,2]){
                ...
                # a hipótese nula H0: a0 = g = 0 (teste F). Alguns autores sugerem testar H0: a0 = 0 dado que g = 0,
                # o que significa testar a0 usando a estatística t. Como os teste podem discordar em alguns casos
                # optamos por realizar somente o primeiro teste (H0: a0 = g = 0).
                if(adf@teststat[2] > adf@cval[2,2]){ #Não
